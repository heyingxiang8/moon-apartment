...
public class FeeController {
    ...
    public Result saveOrUpdateFeeKey(@RequestBody FeeKey feeKey) {
        ...
        feeKey.setIsDeleted(Byte.valueOf("0"));
        boolean isSuccess = feeKeyService.saveOrUpdate(feeKey);
        if(isSuccess){
            return Result.ok();
        }else{
            return Result.fail();
    ...
    public Result saveOrUpdateFeeValue(@RequestBody FeeValue feeValue) {
        feeValue.setUpdateTime(new Date());
        feeValue.setIsDeleted(Byte.valueOf("0"));
        boolean isSuccess = feeValueService.saveOrUpdate(feeValue);
        if(isSuccess){
            return Result.ok();
        }else{
            return Result.fail();
