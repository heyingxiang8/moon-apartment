...
public class AttrController {
    ...
    public Result saveOrUpdateAttrKey(@RequestBody AttrKey attrKey) {
        ...
        attrKey.setIsDeleted(Byte.valueOf("0"));
        boolean isSuccess = attrKeyService.saveOrUpdate(attrKey);
        if(isSuccess){
            return Result.ok();
        }else{
            return Result.fail();
    ...
    public Result saveOrUpdateAttrValue(@RequestBody AttrValue attrValue) {
        attrValue.setUpdateTime(new Date());
        attrValue.setIsDeleted(Byte.valueOf("0"));
        boolean isSuccess = attrValueService.saveOrUpdate(attrValue);
        if(isSuccess){
            return Result.ok();
        }else{
            return Result.fail();
